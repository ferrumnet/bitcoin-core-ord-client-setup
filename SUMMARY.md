# Table of contents

* [👋 Welcome to Bitcoin Core and Ord Setup Tutorial](README.md)

## Overview

* [💡 Why do you need to run a node?](overview/why-do-you-need-to-run-a-node.md)
* [✨ System requirements](overview/system-requirements.md)

## Fundamentals

* [🔁 Run a Bitcoin Node in three easy steps](fundamentals/run-a-bitcoin-node-in-three-easy-steps.md)
* [🛠️ Install required tools, Homebrew, Oh My Zsh, Visual Studio, Rust](fundamentals/install-required-tools-homebrew-oh-my-zsh-visual-studio-rust.md)
* [⚙️ Setup Ord Client on Mac](fundamentals/setup-ord-client-on-mac.md)
* [📝 Contributors](fundamentals/contributors.md)

## Resources

* [⁉️ FAQs](resources/faqs.md)
* [🗃️ Resources and links](resources/resources-and-links.md)
* [📣 Donation and X shout out](resources/donation-and-x-shout-out.md)
